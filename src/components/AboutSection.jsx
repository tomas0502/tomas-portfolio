...
import { Briefcase, Code, User, Download, Calendar, Sparkles, Target, Mail, Star, Github, Send } from 'lucide-react';
...
export const AboutSection = () => {
  ...
  const achievements = [
    { number: "60+", label: "Projects", icon: <Briefcase className="h-5 w-5" />, suffix: "" },
    { number: "7", label: "Years Exp", icon: <Calendar className="h-5 w-5" />, suffix: "+" },
    { number: "99", label: "Success", icon: <Target className="h-5 w-5" />, suffix: "%" },
    { number: "35", label: "Clients", icon: <User className="h-5 w-5" />, suffix: "+" }
  ];

  const techStack = [
    { category: "Frontend", items: ["React", "Next.js", "TypeScript", "JavaScript", "HTML", "Tailwind", "Webflow"] },
  ...
  const socialLinks = [
    { icon: <Mail className="h-5 w-5" />, href: "mailto:uigraslasu@gmail.com" },
    { icon: <Github className="h-5 w-5" />, href: "https://www.github.com/tomas0502" },
    { icon: <Send className="h-5 w-5" />, href: "https://t.me/TomasCooper1004" },
  ...
  const tabContent = {
    ...
    professional: "With 7+ years in full-stack development, I've delivered 15+ successful projects using modern technologies. I specialize in scalable architecture and performance optimization.",
  ...
  const handleDownload = () => {
    ...
    link.href = '/Tomas-resume.pdf'; // Must be in public folder
    link.download = 'Tomas-resume.pdf';
                ...
                <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                  ...
                  <div className="relative flex-shrink-0">
                    <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-2xl overflow-hidden border-4 border-primary/20 shadow-2xl transition-all duration-500 group-hover:border-primary/40 group-hover:scale-105 md:group-hover:scale-110 relative">
                      <img src="/profile-logo.png" alt="Tomas Cooper" className="w-full h-full object-cover" />
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <h2 className="text-2xl sm:text-3xl font-bold mb-1 sm:mb-2">Tomas Cooper</h2>
                ...
                <div className="flex flex-col sm:flex-row border-b border-border mb-4 sm:mb-6">
                  {['personal', 'professional', 'approach'].map(tab => (
                    <button
                      ...
                      className={`cursor-pointer flex-1 py-2 sm:py-3 px-2 sm:px-4 text-sm sm:text-base font-medium transition-all duration-300 ${activeTab === tab ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'}`}
                    >
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <div className="bg-card/50 border border-border rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40 hover:bg-card/60">
              ...
              <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-3 sm:space-y-0">
                <a href="#contact" className="flex-1 block w-full p-1.5 sm:p-2 bg-primary text-primary-foreground rounded-xl text-center font-semibold transition-all duration-300 hover:bg-primary/90 hover:scale-105 hover:shadow-lg group">
                  <div className="flex items-center justify-center gap-2 sm:gap-3"><User className="h-4 sm:h-6 w-5 sm:w-5 group-hover:scale-110 transition-transform duration-300" />Contact Me</div>
                </a>

                {/* Download Button */}
                <button
                  onClick={handleDownload}
                  className="cursor-pointer flex-1 block w-full p-1.5 sm:p-2 border border-border rounded-xl text-center font-semibold transition-all duration-300 hover:bg-accent hover:border-primary/30 hover:scale-105 hover:shadow-lg group"
                >
                  <div className="flex items-center justify-center gap-2 sm:gap-3">
                    <Download className="h-4 sm:h-6 w-5 sm:w-5 group-hover:translate-y-0.5 transition-transform duration-300" />
                    Resume
                  </div>
                </button>
              </div>
            ...
            <div className="bg-card/50 border border-border rounded-3xl p-4 sm:p-6 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40 hover:bg-card/60">
              ...
              <div className="space-y-2 sm:space-y-3">
                {features.map((feature, index) => (
                  <div key={index} className="cursor-pointer flex items-center gap-2 sm:gap-3 p-1 sm:p-2 rounded-lg transition-all duration-300 hover:bg-background/50 hover:scale-105">
